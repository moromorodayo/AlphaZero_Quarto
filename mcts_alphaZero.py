...
class TreeNode(object):
    ...
    def select(self, c_puct):
        ...
        plus bonus u(P).
        Return: A tuple of ((action, koma_int), next_node)
        """
...
class MCTS(object):
    ...
    def _playout(self, state):
        ...
        united_action_probs, leaf_value = self._policy(state)
        # Check for end of game.
        end, winner = state.game_end()
        if not end:
            node.expand(united_action_probs)
        else:
...
class MCTSPlayer(object):
    ...
    def get_action(self, board, temp=1e-3, return_prob=0):
        ...
        if len(sensible_moves) > 0:
            # acts: [where to put,int]
            acts, probs = self.mcts.get_move_probs(board, temp)
            for i in range(len(acts)):
                move_probs[acts[i][0]*16+acts[i][1]] = probs[i]
            #move_probs[list(acts)] = probs
            if self._is_selfplay:
                # add Dirichlet Noise for exploration (needed for
                # self-play training)
                index_move = np.random.choice(
